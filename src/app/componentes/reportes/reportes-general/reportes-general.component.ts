...
export class ReportesGeneralComponent {
  ...
  dataSource: any[] = [];
  displayedColumns: string[] = ['tarea', 'duracion'];
  displayedColumnsDireccion: string[] = ['tarea', 'idUsuario', 'duracion'];
  dataMaxMin: any = {}
  ...
  reporteGeneral(form: any) {
    ...
    this.reportS.getMetricas('procesoTesis', this.seleccionCarrera, form.periodo, fechaInicioFormatted, fechaFinFormatted).subscribe(datos => {
      ...
      if (Object.keys(datos).length === 0) {
        ...
        console.log("ver el reporte general: ", datos);
        this.dataMaxMin = datos.TareasFastaAndLow;

        this.dataSource = Object.entries(datos.averageActivityDurationsDias!).map(([tarea, duracion]) => ({
          tarea,
          duracion
        }));
        ...
        this.barChartData = {
          labels: this.dataSource.map(item => item.tarea),
          datasets: [
            {
              label: 'Duración de tareas',
